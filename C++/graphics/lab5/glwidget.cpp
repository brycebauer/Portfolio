...
void GLWidget::initializeGrid() {
    ...
    gridTrackMatrixLoc = glGetUniformLocation(program, "track");
}
...
void GLWidget::initializeCube() {
    ...
    cubeTrackMatrixLoc = glGetUniformLocation(program, "track");
}
...
void GLWidget::resizeGL(int w, int h) {
    ...
    glUniformMatrix4fv(cubeTrackMatrixLoc, 1, false, value_ptr(trackMatrix));
    ...
    glUniformMatrix4fv(gridTrackMatrixLoc, 1, false, value_ptr(trackMatrix));
}
...
void GLWidget::mousePressEvent(QMouseEvent *event) {
    ...
    povt = pointOnVirtualTrackball(pt);
    lastpoint = povt;
    update();
}
...
void GLWidget::mouseMoveEvent(QMouseEvent *event) {
    ...
    vec3 v2 = normalize(lastpoint);
    vec3 axis = cross(v2,v1);
    if (length(axis) >= .00000001){
        axis = normalize(axis);
        float angle = acos(dot(v1,v2));
        rot = glm::rotate(mat4(1.0), angle, axis);
        trackMatrix = rot * trackMatrix;
        lastpoint = povt;
        //update();
    }
...
vec3 GLWidget::pointOnVirtualTrackball(const vec2 &pt) {
    ...
    p.x = 1*(-1+pt.x*(2.0/width));
    p.y = -((float)height/width)*(1-pt.y*(2.0/height));
    if(p.x*p.x+p.y*p.y <= r*r*.5)
    {
        p.z = sqrt((r*r)-((p.x*p.x)+(p.y*p.y)));
    }
    else
    {
        p.z = (r*r/2)/sqrt(p.x*p.x+p.y*p.y);
    }
    return p;
}
