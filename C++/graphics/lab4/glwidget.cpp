...
void GLWidget::resizeGL(int w, int h) {
    ...
    // Part 1 Done See Below
    // Fills Matrix with 0's
    mat4 persp = glm::perspective((float)(60*M_PI/180), aspect, near, far);
    for(int i = 0; i < 4; i++)
    {
        for(int j = 0; j < 4; j++)
        {
    ...
    // Put the values in the perspective matrix.
    projMatrix[0][0] = 1 / (aspect * tan(60.0*M_PI/180/2.0));
    projMatrix[1][1] = 1 / tan(60.0*M_PI/180/2.0);
  ...
  //  projMatrix = mat4(1/(aspect*tan(6)))
...
void GLWidget::updateModelMatrix() {
    ...
    modelMatrix = {glm::vec4(1,0,0,0),
                   glm::vec4(0,1,0,0),
                   glm::vec4(0,0,1,0),
                   glm::vec4(0,0,0,1)};
    modelMatrix = glm::translate(modelMatrix, glm::vec3(tx, ty, tz));
    modelMatrix = glm::rotate(modelMatrix, glm::radians(180.0f), glm::vec3(rx, ry, rz));
...
void GLWidget::updateViewMatrix() {
    ...
    viewMatrix = glm::lookAt(glm::vec3(10*sin(camAngle), camY, 10*cos(camAngle)),
                             glm::vec3(0,0,0),
                             glm::vec3(0,1,0));
}
